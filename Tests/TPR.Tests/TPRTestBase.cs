...
namespace TPR.Tests
{
    public abstract class TPRTestBase : AbpIntegratedTestBase<TPRTestModule>
    {
        ...
        protected void UsingDbContext(Action<TPRDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TPRDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TPRDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TPRDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TPRDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TPRDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TPRDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TPRDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TPRDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TPRDbContext, Task<T>> func)
        {
            T result;
